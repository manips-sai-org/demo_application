...
int main() {
	...
	while (runloop) {
		...
		if(state == MOTION_1)
		{
			...
			// set goal positions
			Vector3d robot1_desired_position_in_world = Vector3d(0.3, -0.15, 0.3);
			...
			Vector3d robot2_desired_position_in_world = Vector3d(0.3,  0.15, 0.3);
		...
		else if(state == MOTION_2)
		{

			// TODO : Use two hand task to compute augmented object model and grasp matrix
			// temporary :
			Vector3d robot1_desired_position_in_world = Vector3d(0.3, -0.15, 0.2);
			Vector3d robot2_desired_position_in_world = Vector3d(0.3, 0.15, 0.2);
			posori_tasks[0]->_desired_position = robot_pose_in_world[0].linear().transpose()*(robot1_desired_position_in_world - robot_pose_in_world[0].translation());
			posori_tasks[1]->_desired_position = robot_pose_in_world[1].linear().transpose()*(robot2_desired_position_in_world - robot_pose_in_world[1].translation());
			for(int i=0 ; i<n_robots ; i++)
			{
