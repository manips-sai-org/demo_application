...
int main() {
	...
	// debug -----------
	// Sai2Primitives::OTG* otg = new Sai2Primitives::OTG(3,0.001);
	...
	// auto posori_task = new Sai2Primitives::PositionTask(robot, link_name, pos_in_link);
	...
	joint_task->_desired_position = desired_initial_configuration;
	...
	while (runloop) {
		...
		if(state == GOTO_INITIAL_CONFIG)
		{
			...
			if(config_error.norm() < 0.1)
			{
				...
				posori_task->_desired_position(2) -= 0.2;
				state = GOTO_CONTACT;
			}
		...
		else if(state == GOTO_CONTACT)
		{
			...
			// TODO : conditions check for conditions to transition into the next state and implement the transition
			// look at the functions available in PosOriTask.h
